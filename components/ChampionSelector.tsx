...
const ChampionSelector: React.FC<ChampionSelectorProps> = ({ playerName, onSelect, onClose }) => {
  ...
  const filteredChampions = useMemo(() => {
    return CHAMPIONS_LIST.filter(name => name.toLowerCase().includes(searchTerm.toLowerCase()))
    .map(name => ({
      name,
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-black/30">
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-8">
            {filteredChampions.map((champ) => (
              <button key={champ.name} onClick={() => !isConfirming && handleSelect(champ)} className="group flex flex-col items-center gap-4 transition-all active:scale-95">
