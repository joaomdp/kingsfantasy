        ...
        <div className="glass-card rounded-[40px] p-8 border border-white/5 sticky top-24">
          ...
          <div className="grid grid-cols-2 gap-6 mb-12">
            <div className="bg-white/[0.02] p-6 rounded-3xl border border-white/5">
              ...
              <p className="text-2xl font-orbitron font-black text-[#c89b3c]">
                <span className="text-xs mr-1">C$</span>
                {(userTeam.budget / 1000).toFixed(1)}k
              </p>
            </div>
            <div className="bg-white/[0.02] p-6 rounded-3xl border border-white/5">
              <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-2">PAGOS</p>
              <p className="text-2xl font-orbitron font-black text-white">
                <span className="text-xs mr-1">C$</span>
                {((INITIAL_BUDGET - userTeam.budget) / 1000).toFixed(1)}k
              </p>
            </div>
          </div>
          ...
          <div className="relative aspect-square bg-[#050505] rounded-[32px] border border-white/10 overflow-hidden mb-12 shadow-2xl group">
             ...
             {Object.entries(rolePositions).map(([role, pos]) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full border-2 p-0.5 shadow-[0_0_20px_rgba(0,0,0,0.8)] transition-all duration-500 ${p ? 'border-[#c89b3c] bg-black scale-110 shadow-[0_0_15px_rgba(200,155,60,0.4)]' : 'border-white/10 bg-black/80 backdrop-blur-md'}`}>
                      {p ? (
                        <img src={p.image} className="w-full h-full object-cover rounded-full" alt="" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-[7px] font-black text-white/20 uppercase tracking-tighter">
          ...
          <div className="space-y-4">
             ...
             <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-[#c89b3c] shadow-[0_0_15px_rgba(200,155,60,0.5)] transition-all duration-700" style={{ width: `${(hiredCount/5)*100}%` }}></div>
             </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6">
           <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                 ...
                 <input 
                   ...
                   className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 pl-14 pr-6 text-[11px] font-black text-white uppercase placeholder:text-gray-700 focus:outline-none focus:border-[#c89b3c]/40 transition-all shadow-2xl"
                   value={searchTerm}
              ...
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-[11px] font-black text-white uppercase appearance-none focus:outline-none focus:border-[#c89b3c]/40 cursor-pointer min-w-[180px]"
              >
           ...
           <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
              {['TOP', 'JUNGLE', 'MID', 'ADC', 'SUPPORT'].map(role => (
                <button 
                  key={role}
                  onClick={() => setFilterRole(filterRole === role ? 'ALL' : role as Role)}
                  className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterRole === role ? 'bg-[#c89b3c] text-black shadow-lg shadow-[#c89b3c]/20' : 'bg-white/5 text-gray-500 hover:text-white'}`}
                >
                  {role}
                </button>
              ))}
                ...
                <div key={player.id} className="glass-card rounded-[32px] overflow-hidden group hover:bg-white/[0.04] transition-all border border-white/5">
                   <div className="flex items-center p-6 gap-8">
                      <div className="w-20 h-20 shrink-0 rounded-2xl overflow-hidden bg-black border border-white/10 relative">
                         <img src={player.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={player.name} />
                      ...
                      <div className="flex-1">
                         ...
                         <h3 className="text-2xl font-bold text-white tracking-tight uppercase group-hover:text-[#c89b3c] transition-colors leading-none">
                            {player.name}
                         </h3>
                         <div className="flex items-center gap-6 mt-4">
                            <div className="flex flex-col">
                               <span className="text-xs font-orbitron font-bold text-white leading-none">{player.points.toFixed(1)}</span>
                               <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest mt-1">Pontuação</span>
                            </div>
                            <div className="w-px h-6 bg-white/5"></div>
                            <div className="flex flex-col">
                               <span className="text-xs font-orbitron font-bold text-white leading-none">{player.kda}</span>
                               <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest mt-1">KDA Médio</span>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="flex items-center gap-12">
                         <div className="text-right">
                            ...
                            <div className="flex items-baseline justify-end gap-1">
                               <span className="text-[#c89b3c] font-black text-[11px]">C$</span>
                         ...
                         <button 
                           ...
                           className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all ${
                             ...
                             : 'bg-[#c89b3c]/5 text-[#c89b3c] hover:bg-[#c89b3c] hover:text-black border border-[#c89b3c]/20'
                           }`}
                         >
