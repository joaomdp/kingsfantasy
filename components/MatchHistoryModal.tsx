  ...
  return (
    <div className={`fixed left-0 right-0 bottom-0 top-28 z-[300] flex justify-center py-12 md:py-20 transition-all duration-300 ${isClosing ? 'bg-black/0 backdrop-blur-0' : 'bg-black/85 backdrop-blur-md'}`}>
      ...
      <div className={`relative w-full max-w-md h-fit bg-[#0B0411] rounded-[40px] border border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.8)] transition-all duration-500 overflow-hidden ${isClosing ? 'opacity-0 scale-95 translate-y-12' : 'opacity-100 scale-100 translate-y-0 animate-in zoom-in-95'}`}>
        <div className="p-8 border-b border-white/5 flex items-center justify-between bg-gradient-to-br from-[#5E6CFF]/10 to-transparent relative z-10">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-2xl border-2 border-[#5E6CFF]/30 overflow-hidden bg-black shadow-2xl">
               <PlayerImage player={player} className="w-full h-full" />
            </div>
            <div>
              <span className="text-[10px] font-black text-[#5E6CFF] uppercase tracking-widest">{player.role}</span>
        ...
        <div className="p-8 space-y-6 relative z-10">
          ...
          <div className="space-y-4">
            {mockHistory.map((match, idx) => (
              <div key={idx} className="flex items-center justify-between p-5 bg-white/[0.02] rounded-3xl border border-white/5 group hover:border-[#5E6CFF]/30 transition-all duration-300">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[13px] font-black text-white uppercase tracking-tight group-hover:text-[#5E6CFF] transition-colors">{match.champion}</p>
        ...
        <div className="px-8 pb-10 pt-2 relative z-10 text-center">
          <div className="grid grid-cols-2 gap-4 mb-10">
             ...
             <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
                ...
                <p className="text-lg font-orbitron font-black text-[#5E6CFF] shadow-[0_0_10px_rgba(94,108,255,0.2)]">{player.kda}</p>
             </div>
          </div>
          <button onClick={triggerClose} className="inline-block text-white/20 hover:text-[#5E6CFF]/60 text-[9px] font-black uppercase tracking-[0.4em] transition-all">FECHAR RELATÓRIO</button>
        </div>
      </div>
    </div>
  );
